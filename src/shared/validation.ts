...
export function validateModName(modName: string) {
  ...
  if (name.length < 3) {
    errors.push({
      field: "name",
      message: "Mod name must be at least 3 characters long.",
    });
  }
  if (name.length > 64) {
    errors.push({
      field: "name",
      message: "Mod name must be less than 64 characters long.",
    });
  }
  const allowedPattern = /^[\p{Script=Han}a-zA-Z0-9,.¡!¿?$%&()#+;'" _-]+$/u;
...
export function validateModDescription(modDescription: string) {
  ...
  if (description.length < 3) {
    errors.push({
      field: "description",
      message: "Mod description must be at least 3 characters long.",
    });
  }
  if (description.length > 2000) {
    errors.push({
      field: "description",
      message:
        "Mod description must be less than 2000 characters long. (" +
        description.length +
        ")",
    });
  }
...
export function validateModShortDescription(modShortDescription: string) {
  ...
  if (shortDescription.length > 200) {
    errors.push({
      field: "shortDescription",
      message:
        "Mod description must be less than 200 characters long. (" +
        modShortDescription.length +
        ")",
    });
  }
  return errors;
